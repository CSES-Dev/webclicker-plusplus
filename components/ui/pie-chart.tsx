...
export function PieChart( {value, pie_label} : PieData ) {
  ...
  if (!validated_data.success) { //might change zod validation to just set value to 0 if less than 0 and to 100 if greater than 100
    return;
  }
  ...
  const chartData = [
    { browser: "safari", data_score: amount, fill: "#A7F2C2" },
  ]
        ...
        >
          ...
          >
            <PolarGrid
              ...
              radialLines={false}
              stroke="none"
              className="first:fill-chart-5 last:fill-background"
              polarRadius={[0, 0]}
            />
            <RadialBar dataKey="data_score" background cornerRadius={0} className='fill-transparent'/>
