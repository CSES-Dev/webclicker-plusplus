...
const schema = object({
    title: string().min(2),
    color: string().length(7, "Invalid color"),
    days: array(string()).min(1, "Select at least one day"),
    startTime: string().regex(/^([0-1]?[0-9]|2[0-3]):[0-5][0-9]$/, "Invalid Time"),
    endTime: string().regex(/^([0-1]?[0-9]|2[0-3]):[0-5][0-9]$/, "Invalid Time"),
...
export const AddCourseForm = () => {
    ...
    const handleSubmit = (values: zinfer<typeof schema>) => {
