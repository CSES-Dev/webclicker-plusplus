...
export async function createCourseSession(courseId: number, start: Date){
    try{
        if (await findCourseSession(courseId, start)) return {error: "Course session already exists."};
...
export async function findCourseSession(courseId: number, start: Date): Promise<FindCourseSessionResult> {
    try{
